...
export default function Message({ dialogImage, content }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexDirection: sender === user.displayName ? 'row-reverse' : 'row', padding: '5px 20px 0' }} >
      <AccountCircle />
      {isImage ?
        <img src={message} style={{ maxWidth: "50%", borderRadius: 20 }} onClick={() => dialogImage({ show: true, message })} /> :
        <Typography style={{ height: 'fit-content', maxWidth: '100%', backgroundColor: "#2cf590", borderRadius: 15, textAlign: user.displayName ? 'left' : 'right', padding: "5px 20px" }}>{message}</Typography>
      }
